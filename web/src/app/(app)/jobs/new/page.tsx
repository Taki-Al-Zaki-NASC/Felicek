...
export default function PostJob() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="font-serif text-3xl font-semibold">Post a job</h1>
            ...
            {milestones.map((m, i) => (
              <Card key={i} className="flex flex-wrap gap-2 p-2.5">
                <input
                  ...
                  placeholder="Stage"
                  className="min-w-0 flex-1 rounded-field border border-border px-3 py-2 text-sm outline-none focus:border-teal"
                />
                <input
                  value={m.amount}
                  onChange={(e) => setMilestones(milestones.map((x, j) =>
                  ...
                  placeholder="$0.00"
                  className="w-28 rounded-field border border-border px-3 py-2 text-sm outline-none focus:border-teal"
                />
                {milestones.length > 1 && (
